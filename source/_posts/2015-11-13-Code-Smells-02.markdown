...
Classes that suffer from divergent change are ones that get changed a lot. With tools like [Churn]() you can measure how often particular parts of your code needed to change. The more points you find on a class the higer the probablity that divergent change might be one of the reasons. When is this usually happening to a class? Very common symptom is that this object needs to juggle more than one responsibility. The single responsibility principle [SRP]() is most likely almost always a good guideline to write more stable code—not only to prevent this code smell.

more than one reason to change

violation of SRP

kinda opposite of shotgun surgeryo

one class is commonly changed in different ways for different reasons

identify parts that change to together and extract into a separate class
...
class Spectre
  ...
  def lecture_agent(agent)
    puts "I have had a ton of complaints from the Minister of foreign affairs. You have been a very bad boy #{agent.code_name}."
  end
end
```
