...
I know, naming is hard! Anyway, every page allows you to have variables that can be stored right at the top inside a leading and a trailing triple hyphen—3 minus signs top and bottom. I’ll show you:
...
For code that gets exectuted but not “pinted” onto the page you use this:

``` erb
<% %>
```

Otherwise, for return values that you wanna see show up you need to add an equals sign as well. That’s it.
...
This default layout is pretty barebones but provides you all you need to get started. Let’s have a look. Basically this provides you with a skelleton that all your pages need to get going:
...
For any decent app, layouts can be nested of course—which means more than one **yield** block and they can be turned off as well. Maybe we’ll visit that later when we get into building something. For now that’s all you need to know about layouts.
...
Helpers are methods that you can use for a lot of everyday taks in your views. I believe this was pioneerd in Rails land and quickly became ubiquitous for web modern web development. You’ve already seen helpers to include stylesheets and JavaScript files. There’s a lot more where this is coming from though.
...
These simple helpers might come in handy when I quickly wanna prototype something and want to avoid setting up dummy images and text myself. Just drop in a helper in your view, test how it looks and replace it when you have real content for exmaple. Overall, you should look out for code that you’d like to be more concise or that you duplicate over and over again. Helpers are often a good home for that.
...
Using the module approach, you need a separate file for your module. Create a **lib** directory in your root folder—same level as **source** and **build**—and create a file for your helpers.
