...
Let’s close this article with an example how you can slim down your parameters. It gets tedious pretty fast when you have to feed your methods more than one or two arguments. Wouldn’t it be nice to drop in one object instead? That’s exactly what happens if you introduce a parameter object.
...
Let’s go over this simple example. M can assign a new mission and needs needs a mission name, an agent, an objective and is also able to switch between double 0 status.
...
When you look at this and ask what happens when the mission grows in complexity then you’re already onto something. That’s a pain point that you can only solve if you pass in a single object that has all the information you need. More often than not, you can stay away from changing #**assign_new_mission** in the future that way. As a nice side effect, we have now also separated concerns quite nicely—M handles mission asssignments and what not and doesn’t need to know much about the missions internals.
...
So we created a new object, **Mission**, that is soley focused on providing **M** with the information needed to assign a new mission. No need to pass in these pesky parameters yourself. Instead you tell the object to reveal the information you need inside the method itself. Additionally, we also extracted some behaviour—the information how to print into the new **Mission** object. Why should M neeed to know about printing. The new #**print_mission** also benefitted by loosing some weight. What’s also handy about this is that **Mission** might have all sorts of additional methods or states that are nicely encapsulated in one place and ready for you to access.

With this technique you’ll end up with methods that are more concise, they tend to read betteri and you avoid repeating the same group of parameters all over the place. Getting rid of identical groups of parameters is also an important strategy for DRY code. Try to look out for extracting more than just your data. If you can place behaviour in the new class as well you’ll have objects that are more useful—otherwise they’ll quickly start to smell as well.  
...
I’m gonna watch the new Bond now. Hears its not that good though…
